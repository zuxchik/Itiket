const { countrychik } = require("./Country.schema");

const create_Country = async (req, res) => {
    try {
        const { country } = req.body;

        const new_Country = new countrychik({ country });
...
const getCountry = async (req, res) => {
    try {
        const countries = await countrychik.find({});
...
const getCountryById = async (req, res) => {
    try {
        const { id } = req.params;
        const country = await countrychik.findById(id);
        if (!country) {
...
const updateCountry = async (req, res) => {
    try {
        ...
        const updatedCountry = await countrychik.findByIdAndUpdate(countryId, updatedData, { new: true });
